...
static SCIP_DECL_CONSFREE(consFreeMetadata)
{
   ...
   {
      if( data->psd != NULL )
         PS_deallocateParentSetData(scip, &(data->psd));
      if( data->pd != NULL )
...
#else
   /* Use horrible old fashioned way for backwards compatibility */
   SCIP_CALL(SCIPincludeConshdlr(scip, CONSHDLR_NAME, "pseudo-constraint handler, just used to read and UT_write to files",
                                 -1, -1, -1, -1, -1, -1, 0, TRUE, TRUE, TRUE, TRUE, SCIP_PROPTIMING_ALWAYS,
                                 NULL, consFreeMetadata, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                                 consEnfolpMetadata, consEnfopsMetadata, consCheckMetadata, NULL, NULL, NULL, consLockMetadata,
                                 NULL, NULL, NULL, NULL, NULL, consPrintMetadata, NULL, consParseMetadata,
                                 conshdlrdata));
#endif
   return SCIP_OKAY;
}

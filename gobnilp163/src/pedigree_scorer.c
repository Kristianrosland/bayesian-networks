...
static SCIP_Bool strequ(const char* str1, const char* str2) {
   return ( strcmp(str1, str2) == 0 );
}

static SCIP_RETCODE getColumnIndices(SCIP* scip, int* name_column, int* sex_column, int* age_column, int* genome_start_column, int* genome_end_column, int* adult_column)
{
...
static int findNumColumnsNeeded(int name_column, int sex_column, int age_column, int genome_start_column, int genome_end_column, int adult_column)
{
   return MAX(name_column, MAX(sex_column, MAX(age_column, MAX(genome_start_column, genome_end_column))));
}

static SCIP_RETCODE openFile(const char* filename, FILE** file)
{
   if( strequ(filename, "-") )
      (*file) = stdin;
   else
...
static SCIP_RETCODE checkLineLengths(int num_lines, int* line_lengths, int min_columns_needed)
{
   int i;
...
static SCIP_RETCODE readFromFile(SCIP* scip, const char* filename, char* delims, int num_delims, SCIP_Bool merge_delims, char**** lines, int* num_lines, int** line_lengths, int min_columns_needed)
{
   FILE* file;
...
static SCIP_RETCODE extractNameColumn(SCIP* scip, char*** names, char*** lines, int num_lines, int first_row, int column)
{
   int i;
...
static SCIP_RETCODE extractSexColumn(SCIP* scip, char** sexes, char*** lines, int num_lines, int first_row, int column)
{
   int i;
...
static SCIP_RETCODE extractAgeColumn(SCIP* scip, int** ages, char*** lines, int num_lines, int first_row, int column)
{
   int i;
...
static SCIP_RETCODE extractGenomeColumns(SCIP* scip, char**** genomes, char*** lines, int num_lines, int first_row, int start_column, int end_column)
{
   int i, j;
...
static SCIP_RETCODE extractAdultColumn(SCIP* scip, SCIP_Bool** adult, char*** lines, int num_lines, int first_row, int column)
{
   int i;
...
static SCIP_RETCODE extractColumns(SCIP* scip, int num_lines, int first_row, char*** lines, int name_column, int sex_column, int age_column, int genome_start_column, int genome_end_column, int adult_column, char*** names, char** sexes, int** ages, char**** genomes, SCIP_Bool** adult)
{
...
static SCIP_RETCODE freeLines(char**** lines, int* num_lines, int** line_lengths)
{
   int i, j;
...
static SCIP_RETCODE readDataFromFile(SCIP* scip, const char* filename, char* delims, int num_delims, SCIP_Bool merge_delims, int* first_row, int* num_lines, int* num_markers, char*** names, char** sexes, int** ages, char**** genomes, SCIP_Bool** adult, SCIP_Bool* have_ages, SCIP_Bool* have_sexes, SCIP_Bool* have_adult)
{
   char*** lines;
   int* line_lengths;
 ...
 */
static int lookupAlleleIndex(char* allele, char** alleles, int num_alleles)
{
   int i;
 ...
 */
static SCIP_RETCODE countFrequencies(int num_lines, int num_markers, char*** genomes, char* unknown, int** num_alleles, SCIP_Real*** counts, char**** alleles)
{
   int i, j;
 ...
 *
*/
static SCIP_RETCODE readFrequencyFile(SCIP* scip, const char* frequency_file, int num_delims, char* delims, SCIP_Bool merge_delims, int num_markers, int** num_alleles, SCIP_Real*** counts, char**** alleles)
{
   int i, j;
   FILE* file;
 ...
 */
static SCIP_Bool isPossibleParentOf(int child, int parent, int num_markers, char*** genomes, int* ages, SCIP_Bool* adult, int max_mismatches, char* unknown)
{
   int i;
   ...
   if( adult != NULL && adult[parent] == FALSE )
      ...
      return FALSE;
   if( ages != NULL && ages[parent] <= ages[child] )
 ...
 *  @return SCIP_OKAY if the operation succeeded or an appropriate error code otherwise.
 */
static SCIP_RETCODE findPossibleParents(int num_lines, int num_markers, char*** genomes, int* ages, SCIP_Bool* adult, int max_mismatches, char* unknown, int** num_possible_parents, int*** possible_parents)
{
   int i, j;
   ...
   for( i = 0; i < num_lines; i++ )
      for( j = 0; j < num_lines; j++ )
         if( (i != j) && isPossibleParentOf(i, j, num_markers, genomes, ages, adult, max_mismatches, unknown) )
         {
 ...
 *  @return SCIP_OKAY if the operation succeeded or an appropriate error code otherwise.
 */
static SCIP_RETCODE findPossibleParentSets(int num_lines, int* num_possible_parents, int** possible_parents, SCIP_Bool allow_single_parents, SCIP_Bool has_sex_information, char* sexes, int** num_parent_sets, int*** num_parents_in_set, int**** parent_sets)
{
   int i, j, k;
   ...
   for( i = 0; i < num_lines; i++ )
   {
      ...
      for( j = 0; j < num_possible_parents[i]; j++ )
      {
         ...
         {
            if( (j < k) && (!has_sex_information || sexes[possible_parents[i][j]] == 'U' || sexes[possible_parents[i][j]] != sexes[possible_parents[i][k]]) )
            {
...
static SCIP_Real getCountWithErrorMessage(char* allele, int* num_alleles, SCIP_Real** allele_counts, char*** allele_names, int index, int position, char* unknown)
{
   ...
   else
   {
      ...
      if (lookup == -1)
      {
         SCIPerrorMessage("Error: Unknown allele frequency for %s at profile position %d\n", allele, position);
         exit(-1);
      }
...
static SCIP_Bool isHomozygotic(char* allele1, char* allele2)
{
   return strequ(allele1, allele2);
}

static void swapString(char** str1, char** str2)
{
   char* tmp = *str1;
   *str1 = *str2;
   *str2 = tmp;
}
static SCIP_Bool sortAlleles(char** allele1, char** allele2, char* unknown)
{
...
static void sortAllelesAndCounts(char** allele1, char** allele2, SCIP_Real* count1, SCIP_Real* count2, char* unknown) {
...
static SCIP_Real NONEtoAA(SCIP_Real probA)
{
   /* = p(pick A then pick A) */
   return probA*probA;
}
static SCIP_Real NONEtoAB(SCIP_Real probA, SCIP_Real probB)
{
   /* = p(pick A then pick B, or pick B then pick A) */
   return 2*probA*probB;
}
static SCIP_Real AAtoAA(SCIP_Real probA, SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* = p(inherit A without mutation and pick A) */

   return no_mutation * probA ;
}
static SCIP_Real AAtoAB(SCIP_Real probA, SCIP_Real probB, SCIP_Real mutation, SCIP_Real no_mutation)
{
...
static SCIP_Real AAtoBB(SCIP_Real probA, SCIP_Real probB, SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* = p(inherit A with mutation to B and pick B) */

   return mutation * probB ;
}
static SCIP_Real AAtoBC(SCIP_Real probA, SCIP_Real probB, SCIP_Real probC, SCIP_Real mutation, SCIP_Real no_mutation)
{
...
static SCIP_Real ABtoAA(SCIP_Real probA, SCIP_Real probB, SCIP_Real mutation, SCIP_Real no_mutation)
{
...
static SCIP_Real ABtoCC(SCIP_Real probA, SCIP_Real probB, SCIP_Real probC, SCIP_Real mutation, SCIP_Real no_mutation)
{
...
static SCIP_Real ABtoAB(SCIP_Real probA, SCIP_Real probB, SCIP_Real mutation, SCIP_Real no_mutation)
{
...
static SCIP_Real ABtoAC(SCIP_Real probA, SCIP_Real probB, SCIP_Real probC, SCIP_Real mutation, SCIP_Real no_mutation)
{
...
static SCIP_Real ABtoCD(SCIP_Real probA, SCIP_Real probB, SCIP_Real probC, SCIP_Real probD, SCIP_Real mutation, SCIP_Real no_mutation)
{
...
static SCIP_Real AAbecomesAA(SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* 1 stays as A and 2 stays as A */
   return no_mutation * no_mutation;
}
static SCIP_Real AAbecomesBB(SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* 1 becomes B and 2 becomes B */
   return mutation * mutation;
}
static SCIP_Real AAbecomesAB(SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* 1 stays as A and 2 becomes B, or
      1 becomes B and 2 stays as A */
   return 2 * no_mutation * mutation;
}
static SCIP_Real AAbecomesBC(SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* 1 becomes B and 2 becomes C, or
      1 becomes C and 2 becomes B */
   return 2 * mutation * mutation ;
}
static SCIP_Real ABbecomesAA(SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* 1 stays as A and 2 becomes A */
   return no_mutation * mutation ;
}
static SCIP_Real ABbecomesCC(SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* 1 becomes C and 2 becomes C */
   return mutation * mutation ;
}
static SCIP_Real ABbecomesAB(SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* 1 stays as A and 2 stays as B, or
      1 becomes B and 2 becomes A */
   return (no_mutation * no_mutation) + (mutation * mutation) ;
}
static SCIP_Real ABbecomesAC(SCIP_Real mutation, SCIP_Real no_mutation)
{
   /* 1 stays as A and 2 becomes C, or
      1 becomes C and 2 becomes A */
   return (no_mutation * mutation) + (mutation * mutation) ;
}
static SCIP_Real ABbecomesCD(SCIP_Real mutation, SCIP_Real no_mutation)
{
...
static SCIP_Real mutationProbability(char* inherited1, char* inherited2, char* observed1, char* observed2, SCIP_Real mutation, SCIP_Real no_mutation) {
...
static SCIP_Real oneMarkerLikelihoodNoParents(char* allele1, char* allele2, SCIP_Real count1, SCIP_Real count2, char* unknown) {
...
static SCIP_Real oneMarkerLikelihoodOneParent(char* child1, char* child2, SCIP_Real count1, SCIP_Real count2, char* parent1, char* parent2, SCIP_Real pcount1, SCIP_Real pcount2, char* unknown, SCIP_Real mutation, int num_alleles)
{
   SCIP_Real total;
...
static SCIP_Real oneMarkerLikelihoodTwoParents(char* childa, char* childb, SCIP_Real count1, SCIP_Real count2, char* parent1a, char* parent1b, SCIP_Real pcount1a, SCIP_Real pcount1b, char* parent2a, char* parent2b, SCIP_Real pcount2a, SCIP_Real pcount2b, char* unknown, SCIP_Real mutation, int num_alleles)
{
   SCIP_Real total;
 ...
 */
static SCIP_Real computeLikelihoodNoParents(int num_markers, char** genome, int* num_alleles, SCIP_Real** allele_counts, char*** allele_names, char* unknown)
{
   int i;
   SCIP_Real total = 0;
   for( i = 0; i < num_markers / 2; i++ )
   {
      ...
      SCIP_Real count1 = getCountWithErrorMessage(allele1, num_alleles, allele_counts, allele_names, i, 2*1, unknown);
      SCIP_Real count2 = getCountWithErrorMessage(allele2, num_alleles, allele_counts, allele_names, i, 2*1+1, unknown);
 ...
 */
static SCIP_Real computeLikelihoodOneParent(int num_markers, char** child_genome, char** parent_genome, int* num_alleles, SCIP_Real** allele_counts, char*** allele_names, int max_mismatches, char* unknown, int mutation)
{
   int i;
   SCIP_Real total = 0;
   for( i = 0; i < num_markers / 2; i++ )
   {
      ...
      SCIP_Real count1 = getCountWithErrorMessage(child1, num_alleles, allele_counts, allele_names, i, 2*1, unknown);
      SCIP_Real count2 = getCountWithErrorMessage(child2, num_alleles, allele_counts, allele_names, i, 2*1+1, unknown);
      SCIP_Real pcount1 = getCountWithErrorMessage(parent1, num_alleles, allele_counts, allele_names, i, 2*1, unknown);
      SCIP_Real pcount2 = getCountWithErrorMessage(parent2, num_alleles, allele_counts, allele_names, i, 2*1+1, unknown);
 ...
 */
static SCIP_Real computeLikelihoodTwoParents(int num_markers, char** child_genome, char** parent1_genome, char** parent2_genome, int* num_alleles, SCIP_Real** allele_counts, char*** allele_names, int max_mismatches, char* unknown, int mutation)
{
   int i;
   SCIP_Real total = 0;
   for( i = 0; i < num_markers / 2; i++ )
   {
      ...
      SCIP_Real counta = getCountWithErrorMessage(childa, num_alleles, allele_counts, allele_names, i, 2*1, unknown);
      SCIP_Real countb = getCountWithErrorMessage(childa, num_alleles, allele_counts, allele_names, i, 2*1+1, unknown);
      SCIP_Real pcount1a = getCountWithErrorMessage(parent1a, num_alleles, allele_counts, allele_names, i, 2*1, unknown);
      SCIP_Real pcount1b = getCountWithErrorMessage(parent1b, num_alleles, allele_counts, allele_names, i, 2*1+1, unknown);
      SCIP_Real pcount2a = getCountWithErrorMessage(parent2a, num_alleles, allele_counts, allele_names, i, 2*1, unknown);
      SCIP_Real pcount2b = getCountWithErrorMessage(parent2b, num_alleles, allele_counts, allele_names, i, 2*1+1, unknown);
 ...
 *  @return SCIP_OKAY if the operation succeeded or an appropriate error code otherwise.
 */
static SCIP_RETCODE computeLikelihoods(int num_lines, int num_markers, char*** genomes, int max_mismatches, char* unknown, int* num_parent_sets, int** num_parents_in_set, int*** parent_sets, int* num_alleles, SCIP_Real** allele_counts, char*** allele_names, SCIP_Real*** likelihoods, int** new_num_parent_sets, int*** new_num_parents_in_set, int**** new_parent_sets, int mutation)
{
   int i, j, k;
   ...
   for( i = 0; i < num_lines; i++ )
   {
      for( j = 0; j < num_parent_sets[i]; j++ )
      {
         ...
         if( num_parents_in_set[i][j] == 0 )
            likelihood = computeLikelihoodNoParents(num_markers, genomes[i], num_alleles, allele_counts, allele_names, unknown);
         else if( num_parents_in_set[i][j] == 1 )
            likelihood = computeLikelihoodOneParent(num_markers, genomes[i], genomes[parent_sets[i][j][0]], num_alleles, allele_counts, allele_names, max_mismatches, unknown, mutation);
         else
            likelihood = computeLikelihoodTwoParents(num_markers, genomes[i], genomes[parent_sets[i][j][0]], genomes[parent_sets[i][j][1]], num_alleles, allele_counts, allele_names, max_mismatches, unknown, mutation);
...
static SCIP_RETCODE setParentSetData(ParentSetData* psd, int num_items, int* num_parent_sets, int** num_parents_in_set, int*** parent_sets, char** names)
{
   psd->n = num_items;
...
static SCIP_RETCODE setPedigreeData(SCIP* scip, int num_items, int* ages, char* sexes)
{
...
static SCIP_RETCODE setPropertyData(SCIP* scip, PropertyData* prop, int num_items, const char* filename, const char* frequency_file, int num_markers, SCIP_Bool allow_single_parents, SCIP_Bool have_ages, SCIP_Bool have_sexes, SCIP_Bool have_adults, int* ages, char* sexes, SCIP_Bool* adult, char*** genomes, int* num_alleles, SCIP_Real** allele_counts, char*** allele_names)
{
   int i, j;
...
static SCIP_RETCODE storeResults(SCIP* scip, ParentSetData* psd, PropertyData* prop, SCIP_Real*** scores, int num_items, int* num_parent_sets, int** num_parents_in_set, int*** parent_sets, char** names, int* ages, char* sexes, SCIP_Bool* adult, char*** genomes, const char* filename, const char* frequency_file, int num_markers, SCIP_Bool allow_single_parents, SCIP_Bool have_ages, SCIP_Bool have_sexes, SCIP_Bool have_adults, int* num_alleles, SCIP_Real** allele_counts, char*** allele_names, SCIP_Real** likelihoods)
{
...
static SCIP_RETCODE freeMemory(SCIP* scip, char**** genomes, char*** names, int** ages, char** sexes, SCIP_Bool** adult, int** num_alleles, SCIP_Real*** allele_counts, char**** allele_names, int** num_possible_parents, int*** possible_parents, int** num_parent_sets, int*** num_parents_in_set, int**** parent_sets, int** new_num_parent_sets, int*** new_num_parents_in_set, int**** new_parent_sets, SCIP_Real*** likelihoods, SCIP_Bool have_ages, SCIP_Bool have_sexes, SCIP_Bool have_adults, int first_row, int num_markers, int num_lines, char** unknown)
{
   int i,j;
 ...
 */
SCIP_RETCODE PS_readProblemInGenomeFormat(SCIP* scip, const char* filename, int num_delims, char* delims, SCIP_Bool merge_delims, const char* frequency_file, ParentSetData* psd, SCIP_Real*** scores, PropertyData* prop)
{
   int first_row = 0;
   int num_markers;
   int num_lines;
   ...
   if( strequ(frequency_file, "") )
      SCIP_CALL( countFrequencies(num_lines - first_row, num_markers, genomes, unknown, &num_alleles, &allele_counts, &allele_names) );
   else
   ...
   SCIP_CALL( findPossibleParents(num_lines - first_row, num_markers, genomes, ages, adult, maxmismatches, unknown, &num_possible_parents, &possible_parents) );
   SCIP_CALL( findPossibleParentSets(num_lines - first_row, num_possible_parents, possible_parents, allow_single_parents, have_sexes, sexes, &num_parent_sets, &num_parents_in_set, &parent_sets) );
   ...
   /* Free memory. */
   SCIP_CALL( freeMemory(scip, &genomes, &names, &ages, &sexes, &adult, &num_alleles, &allele_counts, &allele_names, &num_possible_parents, &possible_parents, &num_parent_sets, &num_parents_in_set, &parent_sets, &new_num_parent_sets, &new_num_parents_in_set, &new_parent_sets, &likelihoods, have_ages, have_sexes, have_adults, first_row, num_markers, num_lines, &unknown) );

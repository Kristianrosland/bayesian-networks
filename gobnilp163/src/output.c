 ...
 */
static SCIP_RETCODE printAverages(SCIP* scip, ParentSetData* psd, char* filename, char* format)
{
   int i, j;
   SCIP_Real** Scores;
   SCIP_Bool** selected;
   ...
   if( psd != NULL )
   {
      ...
      for( i = 0; i < psd->n; i++ )
         for( j = 0; j < psd->nParentSets[i]; j++ )
         {
            Scores[i][j] = MA_getAverageValue(psd->PaVars[i][j]);
            if( Scores[i][j] != 0 )
               selected[i][j] = TRUE;
            else
               selected[i][j] = FALSE;
         }
      SCIP_CALL( printToFile(scip, psd, Scores, selected, filename, format, MA_getTotalAveragesTime(), MA_getTotalAveragesScore(), NULL) );
 ...
 *  Any occurance of @c <probname> will be replaced with the basename of the problem.
 *
...
static char* createFilename(char* filename, const char* probname, int nbns, int iteration)
{
   ...
   else
   {
      ...
      if( nbns == 1 || BN_exitBeforeSolving() )
...
SCIP_RETCODE IO_printProblem(SCIP* scip, int run)
{
   ...
   char* cipfile;
   char* outputfile;
   SCIP_Bool presolvecip;

   SCIP_CALL( SCIPgetBoolParam(scip,"gobnilp/presolvecip",&presolvecip) );

   SCIPgetIntParam(scip, "gobnilp/nbns", &nbns);
   SCIPgetStringParam(scip, "gobnilp/outputfile/cip", &cipfile);
   outputfile = createFilename(cipfile, SCIPgetProbName(scip), nbns, run);
   ...
   else
   {
      ...
      {
         SCIPpresolve(scip);
         SCIP_CALL( SCIPwriteTransProblem(scip, outputfile, "cip", FALSE) );
      }
      else
         SCIP_CALL( SCIPwriteOrigProblem(scip, outputfile, "cip", FALSE) );
      free(outputfile);
      return SCIP_OKAY;
   }
 ...
 */
SCIP_RETCODE IO_doIterativePrint(SCIP* scip, ParentSetData* psd, int run)
{
   char* solfile;
   char* dotfile;
   char* pedfile;
   ...
   if( avgoutputoffset > -1 && (run + 1) >= avgoutputoffset )
   {
      if( (run - avgoutputoffset + 1) % avgoutputstep == 0 )
      {
         ...
         SCIP_CALL( printAverages(scip, psd, run_avgsolfile, (char*)"legacy") );
         SCIP_CALL( printAverages(scip, psd, run_avgdotfile, (char*)"dot") );
         SCIP_CALL( printAverages(scip, psd, run_avgpedfile, (char*)"pedigree") );
         SCIP_CALL( printAverages(scip, psd, run_avgsatfile, (char*)"scoreandtime") );
         SCIP_CALL( printAverages(scip, psd, run_avgmatfile, (char*)"adjacencymatrix") );

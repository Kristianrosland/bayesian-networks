   ...
   )
{
   ...
   for( i = 0; i < psd->n; ++i )
   {
      for( j = 0; j < SPARSE_MATRIX_SIZE; ++j )
      {
         if( (psd->arrow[i][j]).js != NULL )
            SCIPfreeMemoryArray(scip, &((psd->arrow[i][j]).js));
         if( (psd->arrow[i][j]).vars != NULL )
            SCIPfreeMemoryArray(scip, &((psd->arrow[i][j]).vars));
         if( (psd->edge[i][j]).js != NULL )
            SCIPfreeMemoryArray(scip, &((psd->edge[i][j]).js));
         if( (psd->edge[i][j]).vars != NULL )
            SCIPfreeMemoryArray(scip, &((psd->edge[i][j]).vars));
      }
      SCIPfreeMemoryArray(scip, &(psd->arrow[i]));
      SCIPfreeMemoryArray(scip, &(psd->edge[i]));
   }
   SCIPfreeMemoryArray(scip, &(psd->arrow));
   SCIPfreeMemoryArray(scip, &(psd->edge));
   ...
   )
{
   ...
   SCIP_CALL( SCIPallocMemoryArray(scip, &(psd->arrow), psd->n) );
   SCIP_CALL( SCIPallocMemoryArray(scip, &(psd->edge), psd->n) );
   for( i = 0; i < psd->n; ++i )
   {
      SCIP_CALL( SCIPallocMemoryArray(scip, &(psd->arrow[i]), SPARSE_MATRIX_SIZE) );
      SCIP_CALL( SCIPallocMemoryArray(scip, &(psd->edge[i]), SPARSE_MATRIX_SIZE) );

      for( j = 0; j < SPARSE_MATRIX_SIZE; ++j )
      {
         ((psd->arrow)[i][j]).js = NULL;
         ((psd->arrow)[i][j]).vars = NULL;
         ((psd->arrow)[i][j]).next = 0;
         ((psd->arrow)[i][j]).length = 0;
         ((psd->edge)[i][j]).js = NULL;
         ((psd->edge)[i][j]).vars = NULL;
         ((psd->edge)[i][j]).next = 0;
         ((psd->edge)[i][j]).length = 0;
   ...
   )
{
   ...
   bucket = &(matrix[i][j % SPARSE_MATRIX_SIZE]);
   
   assert(bucket != NULL);
   assert(bucket->length == 0 || bucket->js != NULL);
   assert(bucket->length == 0 || bucket->vars != NULL);
   assert(bucket->next <= bucket->length);

   for( l = 0; l < bucket->next; ++l )
      if( bucket->js[l] == j )
         return bucket->vars[l];
   ...
   )
{
   ...
   if( ! (bucket->next < bucket->length) )
   {
      (bucket->length) += BUCKET_BLOCK_LENGTH;
      SCIP_CALL( SCIPreallocMemoryArray(scip, &(bucket->js), bucket->length) );
      SCIP_CALL( SCIPreallocMemoryArray(scip, &(bucket->vars), bucket->length) );
   }

   assert(bucket->js != NULL);
   assert(bucket->vars != NULL);
   assert(bucket->next < bucket->length);

   bucket->js[bucket->next] = j;
   bucket->vars[bucket->next] = arrow_i_j;
   (bucket->next)++;
 ...
 *  @param max The parameter's msximum value.
 *  @param value The parameter's initial value.
 *  @return SCIP_OKAY if the operation suceeded.  Otherwise an appropriate error message.
 */
...
/** Adds an real valued parameter to those recognised by SCIP.
 *
...
static SCIP_RETCODE readNextLine(SCIP* scip, FILE* file, int* length, char** buffer, char** status)
{
   ...
   /* Repeat until we have a new line in the buffer */
   while( strchr(*buffer, '\n') == NULL )
   {
      int pos = (*length);
...
SCIP_RETCODE UT_readFileAndSplit(SCIP* scip, FILE* file, char* splitters, int num_splitters, SCIP_Bool multiple_splitters_as_one, char**** lines, int* num_lines, int** line_lengths)
{
   ...
   SCIP_CALL( SCIPallocMemoryArray(scip, &line, line_length) );

...
#define HEUR_FREQ             0                                /**< The heursitic is called only in the root */
...
struct SCIP_HeurData
{
   ...
   unsigned int seed;            /**< The seed to use for random values. */
...
SCIP_DECL_HEURINIT(heurInitSinks)
{
   ...
   if( strcmp(filesols, "") != 0 )
      ...
      heurdata->file = NULL;
   heurdata->seed = (unsigned int) seed;
...
/* sink finding algorithm: for each selected parentset I(v<-W)
   ...
   of I(v<-W). Algorithm greedily attempts to minimise total loss
*/
...
SCIP_DECL_HEUREXEC(heurExecSinks)
{
   ...
   SCIP_Real** lpsolvals;    /** need to store the LP solution
                                 asking for values from NULL (at the wrong point) will render the pseudo-solution */
   ...
   int* bestparents = NULL;    /** bestparents[j] is the index for the best
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   for( run = 0; run < heurdata->nruns; ++run )
   {
      ...
      {
         /* assume parent sets are ordered with best score first */
         bestparents[i] = 0;
         not_sinks[i] = i;
         sink[i] = FALSE;
         loss_lb[i] = 0.0;
      ...
      SCIPpermuteIntArray(not_sinks,0,sinks_to_choose,&(heurdata->seed));
      ...
      for( j = 0; j < psd->n; ++j )
      {
         while( SCIPvarGetUbLocal(psd->PaVars[j][bestparents[j]]) < 0.5 )
         {
            loss_lb[j] += lpsolvals[j][bestparents[j]];
            bestparents[j]++;
         }
         /* can't initially have already ruled out all parent sets! */
      ...
      while( sinks_to_choose )
      {
         ...
         else
         {
            ...
            {
               arrowvar = get_arrow(psd,bestvariable,psd->ParentSets[bestvariable][k][l]);
               if( arrowvar == NULL || SCIPvarGetUbLocal(arrowvar) < 0.5 ) 
                  goto ABORT;
               SCIP_CALL( SCIPsetSolVal(scip, sol, arrowvar, 1.0) );

               edgevar = get_edge(psd,bestvariable,psd->ParentSets[bestvariable][k][l]);
               if( edgevar == NULL || SCIPvarGetUbLocal(edgevar) < 0.5 ) 
                  goto ABORT;
               SCIP_CALL( SCIPsetSolVal(scip, sol, edgevar, 1.0) );
            }
         }
            ...
            do
            {
               ...
               */
               ...
               if( !current_bestparents_for_j_allowed )
               {
                  ...
                  bestparents[j]++;
                  while( bestparents[j] < psd->nParentSets[j] && SCIPvarGetUbLocal(psd->PaVars[j][bestparents[j]]) < 0.5 )
                  {

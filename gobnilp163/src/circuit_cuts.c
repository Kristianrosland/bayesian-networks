 ...
 *
 *  The tecchnique involves finding all elementary circuits in the current
...
static SCIP_Bool ADD_FRACTIONAL_CUTS = TRUE;
/** Whether cluster cuts should be added. */
static SCIP_Bool ADD_CLUSTER_CUTS = TRUE;
/** Whether cycle cuts should be added. */
static SCIP_Bool ADD_CYCLE_CUTS = FALSE;
/** Whether cluster cuts should be added to the cut pool. */
static SCIP_Bool ADD_CLUSTER_CUTS_TO_POOL = FALSE;
/** Whether cycle cuts should be added to the cut pool. */
static SCIP_Bool ADD_CYCLE_CUTS_TO_POOL = FALSE;
/** The maximum length of cycle to look for. */
static int MAX_CYCLE_LENGTH = 6;
...
SCIP_RETCODE CC_addParams(SCIP* scip)
{
   ...
   SCIP_CALL(SCIPaddIntParam(scip,
                             ...
                             NULL, FALSE, MAX_CYCLE_LENGTH, 0, INT_MAX, NULL, NULL));
...
SCIP_RETCODE CC_initialise(SCIP* scip, CircuitCutsStorage* ccs, ParentSetData* data)
{
   ...
   /* Read the parameters */
   SCIP_CALL( SCIPgetBoolParam(scip, "gobnilp/circuit_cuts/add_cluster_cuts",          &ADD_CLUSTER_CUTS) );
   SCIP_CALL( SCIPgetBoolParam(scip, "gobnilp/circuit_cuts/add_cluster_cuts_to_pool",  &ADD_CLUSTER_CUTS_TO_POOL) );
   SCIP_CALL( SCIPgetBoolParam(scip, "gobnilp/circuit_cuts/add_cycle_cuts",            &ADD_CYCLE_CUTS) );
   SCIP_CALL( SCIPgetBoolParam(scip, "gobnilp/circuit_cuts/add_cycle_cuts_to_pool",    &ADD_CYCLE_CUTS_TO_POOL) );
   SCIP_CALL( SCIPgetBoolParam(scip, "gobnilp/circuit_cuts/use_fractional",            &ADD_FRACTIONAL_CUTS) );
   SCIP_CALL( SCIPgetIntParam(scip, "gobnilp/circuit_cuts/max_length",                &MAX_CYCLE_LENGTH) );
   ...
   ccs->cycles = VectorListCreate(MAX_CYCLES);
 ...
 *
 *  @param scip The SCIP instance the separation took place in.
 *
 ...
 *  @return SCIOP_OKAy if the procedure worked correctly, or an error otherwsie.
 */
 ...
 */
static SCIP_Bool circuit(SCIP* scip, Vector* nodes, int current_index, int start_index, VectorList* b, SCIP_Bool* blocked, Stack* s, SCIP_Real** adj_matrix, VectorList* cycles, SCIP_Real weight)
{
   int i, j;
   ...
   )
{
   ...
   for( i = 0; i < cycles->size; i++ )
   {
...
#else
      SCIP_CALL( SCIPcreateEmptyRow(scip, &cluster_cut, "clustercut", -SCIPinfinity(scip), SCIPinfinity(scip), FALSE, FALSE, TRUE) );
      SCIP_CALL( SCIPcreateEmptyRow(scip, &cycle_cut, "cyclecut", -SCIPinfinity(scip), SCIPinfinity(scip), FALSE, FALSE, TRUE) );
#endif
      ...
      {
#if SCIP_VERSION >= 310
         SCIP_CALL( SCIPaddCut(scip, sol, cluster_cut, forcecuts, cutoff) );
         if( *cutoff )
         {
...
#else
         SCIP_CALL( SCIPaddCut(scip, sol, cluster_cut, forcecuts) );
#endif
      ...
      {
#if SCIP_VERSION >= 310
         SCIP_CALL( SCIPaddCut(scip, sol, cycle_cut, forcecuts, cutoff) );
         if( *cutoff )
         {
            SCIP_CALL( SCIPreleaseRow(scip, &cluster_cut) );
            SCIP_CALL( SCIPreleaseRow(scip, &cycle_cut) );
...
#else
         SCIP_CALL( SCIPaddCut(scip, sol, cycle_cut, forcecuts) );
#endif
      }
      if( ADD_CLUSTER_CUTS_TO_POOL )
      {
         SCIP_CALL( SCIPaddPoolCut(scip, cluster_cut) );
      }
      if( ADD_CYCLE_CUTS_TO_POOL )
      {
   ...
   )
{
   ...
   if( ADD_CLUSTER_CUTS || ADD_CYCLE_CUTS || ADD_CLUSTER_CUTS_TO_POOL || ADD_CYCLE_CUTS_TO_POOL )
   {
      int i;
      int j;
      int k;
      ...
      for( i = 0; i < psd->n; i++ )
         for( j = 0; j < psd->n; j++ )
            ...
            else if( ADD_FRACTIONAL_CUTS == FALSE )
            {
               ccs->adj_matrix[i][j] = 0;
            }
      ...
      addCuts(scip, conshdlr, psd, sol, ccs->included_cluster, ccs->excluded_cluster, ccs->included_cycle, ccs->excluded_cycle, ccs->cycles, forcecuts, found_efficacious_ptr, cutoff);

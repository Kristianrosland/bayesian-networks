 ...
 */
SCIP_RETCODE PS_deallocateParentSetData(SCIP* scip, ParentSetData** psd)
{
   int i;
   int k;
...
SCIP_RETCODE PS_copyParentSetData(SCIP* scip, ParentSetData* original, ParentSetData** duplicate)
{
   ...
   for( i = 0; i < original->n; i++ )
   {
      ...
      for( j = 0;  j < original->nParentSets[i]; j++ )
         SCIP_CALL( SCIPallocMemoryArray(scip, &((*duplicate)->ParentSets[i][j]), original->nParents[i][j]) );
   }
   ...
   for( i = 0; i < original->n; i++ )
   {
      ...
      for( j = 0;  j < original->nParentSets[i]; j++ )
      {
         (*duplicate)->nParents[i][j] = original->nParents[i][j];
         (*duplicate)->PaVars[i][j] = original->PaVars[i][j];
         for( k = 0;  k < original->nParents[i][j]; k++ )
            (*duplicate)->ParentSets[i][j][k] = original->ParentSets[i][j][k];
      }
 ...
 *
 *  @param scip The SCIP instance the parent set dtaa is used in.

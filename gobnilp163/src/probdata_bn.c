   ...
   )
{
   ...
   /* pos(j) - pos(i) + n*I(j is a parent of i) =< n-1 */
   ...
   for( i = 0; i < psd->n; ++i )
   {
      for( j = 0; j < psd->n; ++j )
      {
         ...
         SCIP_CALL(SCIPcreateConsLinear(scip, &cons, s, 0, NULL, NULL, -SCIPinfinity(scip), (psd->n) - 1,
               TRUE,
               TRUE,
         ...
         for( k = 0; k < psd->n; ++k )
         {
            SCIP_CALL( SCIPaddCoefLinear(scip, cons, posind[i][k], -k) );
            SCIP_CALL( SCIPaddCoefLinear(scip, cons, posind[j][k], k) );
         }
         arrow_i_j = get_arrow(psd,i,j);
         if( arrow_i_j != NULL )
            SCIP_CALL( SCIPaddCoefLinear(scip, cons, arrow_i_j, psd->n) );
   ...
   )
{
   ...
   for( i = 0; i < psd->n; ++i )
   {
      ...
      /*  SCIP_CALL(  SCIPchgVarBranchPriority(scip,pos[i],10)  );*/
   }

...
typedef struct
{
   int* js;          /**< js[i] is ith j value */
   SCIP_VAR** vars;  /**< vars[i] is ith variable */
   int next;         /**< all items in bucket has index < next */
   int length;       /**< current space allocated for bucket */
} BUCKET;            
...
typedef struct
{
   ...
   BUCKET** arrow;       /**<  `arrow[i][j]` is an indicator variable for an arrow from j to i */
   BUCKET** edge;        /**<  `edge[i][j]`  is an indicator variable for an edge (in either direction) between i and j. Only non-NULL for j > i */
} ParentSetData;

extern SCIP_RETCODE PS_deallocateParentSetData(SCIP* scip, ParentSetData** psd);
